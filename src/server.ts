...
import { loadConfig, getConfigPath, ToolConfig, Config, ConfigManager } from "./config";
...
function createEditorOptimizedResponse(content: string, metadata: any) {
  ...
  const template = responseTemplates[intelligenceType as keyof typeof responseTemplates] || responseTemplates.ANALYTICAL_INTELLIGENCE;
...
function generateContextExpansion(metadata: any) {
  return {
    technicalContext: {
...
function getPrimaryUseCase(intelligenceType: string): string {
...
function getIntegrationPatterns(intelligenceType: string): string[] {
...
function categorizeToolByName(toolName: string): string {
...
function classifyError(error: any): {
  type: 'NETWORK' | 'AUTH' | 'RATE_LIMIT' | 'MODEL' | 'CONFIG' | 'UNKNOWN';
  retryable: boolean;
...
function determineIntelligenceType(toolName: string, toolConfig: ToolConfig): string {
...
function createToolHandler(toolName: string, configGetter: () => Config) {
    ...
    try {
      ...
      return await circuitBreaker.execute(async () => {
        return await executeWithRetry(async () => {
          ...
          const contextSummary = memoryManager.generateContextSummary(toolName, 5, currentToolConfig.model);
          
          let conversation = [
            { role: "system", content: buildToolPrompt(currentToolConfig, config.tools, contextSummary) },
          ...
          while (maxIterations-- > 0) {
            ...
            for (let attemptIndex = 0; attemptIndex < modelConfigs.length && attemptIndex < maxFallbackAttempts; attemptIndex++) {
              ...
              try {
                ...
                modelResponse = await executeWithRetry(
                  () => executeModel(
                    modelConfig,
                    conversation.map(m => `${m.role}: ${m.content}`).join("\n\n")
                  ),
                  DEFAULT_RETRY_CONFIG,
                  `${toolName} ${isFallback ? 'fallback' : 'primary'} model execution (${modelId}, iteration ${iterationCount})`
                );
            ...
            try {
              ...
              const toolResponse = await executeWithRetry(
                () => executeModel(
                  subToolModelConfig,
                  `${subToolConfig.prompt}\n\n${toolCall.query}`
                ),
                DEFAULT_RETRY_CONFIG,
                `Sub-tool ${toolCall.tool} execution`
              );
          ...
          await memoryManager.addMemory(
            ...
            [toolName, successfulModelId || currentToolConfig.model],
            { timestamp: Date.now() }
          );
          ...
          const enhancedResponse = createEditorOptimizedResponse(
            finalResponse,
            {
              ...
              contextMetadata: {
                ...
                relatedTools: Object.keys(config.tools).filter(name => 
                  name !== toolName && config.tools[name].description.toLowerCase().includes(
                    currentToolConfig.description.toLowerCase().split(' ')[0]
                  )
                ),
...
let registeredTools: Set<string> = new Set();
...
export async function startServer() {
  ...
  try {
    ...
    await memoryManager.initializeSession();
...
async function registerTools(server: McpServer, config: Config) {
  try {
    ...
    for (const toolName in config.tools) {
      if (!registeredTools.has(toolName)) {
        ...
        server.tool(
          ...
          createToolHandler(toolName, () => configManager?.getConfig() || config)
        );
