...
// Model capability profiles for cross-model intelligence adaptation
interface ModelCapabilityProfile {
  contextWindow: number;
  strengths: string[];
  preferredFormats: string[];
  responseStyle: string;
  crossModelCompatibility: string;
}
...
const MemorySessionSchema = z.object({
  ...
  sharedContext: z.record(z.string(), z.any()).optional()
});
...
export class MemoryManager {
  ...
  private maxMemorySize: number = 1000; // Max entries per session
  ...
  private getSessionPath(sessionId: string): string {
    return join(this.memoryDir, `${sessionId}.json`);
  }
  ...
  private async loadSession(sessionId: string): Promise<MemorySession> {
    ...
    try {
      const rawData = readFileSync(sessionPath, "utf8");
  ...
  private async saveSession() {
    ...
    try {
      writeFileSync(sessionPath, JSON.stringify(this.currentSession, null, 2));
  ...
  // Get recent memories
  public getRecentMemories(count: number = 10, tool?: string): MemoryEntry[] {
    if (!this.currentSession) return [];
  ...
  public searchMemories(query: string, tool?: string): MemoryEntry[] {
    if (!this.currentSession) return [];
  ...
  public getMemoriesByTags(tags: string[]): MemoryEntry[] {
    if (!this.currentSession) return [];
  ...
  // Get shared context
  public getSharedContext(key?: string): any {
  ...
  public generateContextSummary(tool: string, maxEntries: number = 5, targetModel?: string): string {
  ...
  private generateCrossModelContext(currentTool: string, memories: MemoryEntry[], targetModel?: string): string {
  ...
  private getModelProfile(modelName?: string): ModelCapabilityProfile {
    if (!modelName) return MODEL_PROFILES.default;
    return MODEL_PROFILES[modelName] || MODEL_PROFILES.default;
  }

  // Analyze capability transfer between models
  private analyzeCapabilityTransfer(memories: MemoryEntry[], targetProfile: ModelCapabilityProfile) {
    ...
    const alignedStrengths = targetProfile.strengths.filter(strength => 
      sourceCapabilities.has(strength) || this.isCompatibleCapability(strength, Array.from(sourceCapabilities))
    );
  ...
  private isCompatibleCapability(targetCapability: string, sourceCapabilities: string[]): boolean {
    ...
    return compatibilityMap[targetCapability]?.some(compat => 
      sourceCapabilities.includes(compat)
    ) || false;
  }

  // Determine optimal format adaptation strategy
  private determineFormatAdaptation(memories: MemoryEntry[], targetProfile: ModelCapabilityProfile): string {
  ...
  private assessQualityPreservation(memories: MemoryEntry[], targetProfile: ModelCapabilityProfile): string {
  ...
  private adaptContextForModel(memories: MemoryEntry[], targetProfile: ModelCapabilityProfile): string {
  ...
  private generateCrossModelPerformanceIntelligence(memories: MemoryEntry[], targetProfile: ModelCapabilityProfile): string {
    const perfSections = [];
  ...
  private generateModelSpecificRecommendations(tool: string, targetProfile: ModelCapabilityProfile): string {
  ...
  private assessTransferQuality(sourceProfile: ModelCapabilityProfile, targetProfile: ModelCapabilityProfile): string {
    const strengthOverlap = sourceProfile.strengths.filter(s => 
      targetProfile.strengths.includes(s)
    ).length;
    
    const formatCompatibility = sourceProfile.preferredFormats.some(f => 
      targetProfile.preferredFormats.includes(f)
    );
  ...
  private analyzeModelPerformance(memories: MemoryEntry[]): Record<string, any> {
  ...
  private calculateContextEfficiency(memories: MemoryEntry[], targetProfile: ModelCapabilityProfile): number {
    const totalTokens = memories.reduce((sum, m) => 
      sum + m.prompt.length + m.response.length, 0
    );
  ...
  private calculateCompatibilityScore(memories: MemoryEntry[], targetProfile: ModelCapabilityProfile): number {
    let totalScore = 0;
    let scoreCount = 0;
    ...
    memories.forEach(memory => {
      ...
      const strengthScore = (sourceProfile.strengths.filter(s => 
        targetProfile.strengths.includes(s)
      ).length / Math.max(sourceProfile.strengths.length, targetProfile.strengths.length)) * 40;

      // Format compatibility (30% weight)
      const formatScore = sourceProfile.preferredFormats.some(f => 
        targetProfile.preferredFormats.includes(f)
      ) ? 30 : 15;
  ...
  private generateExpandedContext(currentTool: string, memories: MemoryEntry[]): string {
    const contextSections = [];
  ...
  private analyzeToolUsagePatterns(): Record<string, {count: number, avgDuration: number}> {
    const patterns: Record<string, {durations: number[], count: number, avgDuration: number}> = {};
  ...
  private analyzeToolRelationships(memories: MemoryEntry[]): Array<{fromTool: string, toTool: string, relationship: string}> {
    const relationships = [];
  ...
  private determineRelationship(from: MemoryEntry, to: MemoryEntry): string {
  ...
  private calculatePerformanceMetrics(memories: MemoryEntry[]) {
  ...
  public getCurrentSessionInfo(): { sessionId: string; entryCount: number; startTime: number } | null {
    if (!this.currentSession) return null;
