...
server.tool("run", {
    query: zod_1.z.string().describe("User's query or task"),
    configPath: zod_1.z.string().optional().describe("Path to config file")
}, async ({ query, configPath }) => {
    try {
        // Load configuration
        const config = (0, config_1.loadConfig)(configPath || (0, config_1.getConfigPath)());
        ...
        const mainTool = Object.values(config.tools).find(tool => tool.model === config.mainModel);
        if (!mainTool) {
            throw new Error(`No tool found for main model '${config.mainModel}'`);
        }
        // Prepare initial prompt
        let conversation = [
            { role: "system", content: (0, models_1.buildToolPrompt)(mainTool, config.tools) },
        ...
        while (maxIterations-- > 0) {
            // Execute the model
            const modelResponse = await (0, models_1.executeModel)(mainModelConfig, conversation.map(m => `${m.role}: ${m.content}`).join("\n\n"));
            ...
            try {
                ...
                // Execute tool
                const toolResponse = await (0, models_1.executeModel)(toolModelConfig, `${toolConfig.prompt}\n\n${toolCall.query}`);
                // Add to conversation
